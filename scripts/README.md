...
`tests/fixtures/evidence/`의 경계 사례 22개를 만든다. 정상 10개는 월 수치로, 오류 12개는 정상 ①에서 한 곳만 바꿔 만든다. 사례 수치와 기대 결과는 `1근거계산계층계획.md` 6장. `build_all()`은 테스트가 커밋된 파일과 비교할 때도 쓴다. 생성 목록에 없는 JSON이 폴더에 남아 있으면 종료 코드 1.
