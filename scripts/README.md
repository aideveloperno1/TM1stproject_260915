...
| `build_demo_evidence.py` | 화면 시연에 쓰는 **가짜(합성) 근거 파일**을 만듦. 누가 봐도 가상인 금액으로 만듦 | 시연 화면에 보일 가짜 자료를 바꿀 때 |
| `build_fixtures.py` | 자동 시험에 쓰는 **경계 사례 근거 파일 22개**를 만듦 (정상 파일, 일부러 틀린 파일) | 시험 사례를 추가·변경할 때 |
...
`tests/fixtures/evidence/`의 경계 사례 22개를 만든다. 월 수치로 직접 만드는 10개와 정상 ①에서 한 곳만 바꾸는 12개로 나뉜다(12개 중 `denominator_zero`는 로드 성공, 나머지 11개는 로드 실패). 사례 수치와 기대 결과는 `1근거계산계층계획.md` 6장. `build_all()`은 테스트가 커밋된 파일과 비교할 때도 쓴다. 생성 목록에 없는 JSON이 폴더에 남아 있으면 종료 코드 1.
