...
def find_violations(repo_root: Path, files: Iterable[str]) -> list[Violation]:
    ...
    for rel in files:
        ...
        if rel.startswith("private/") and rel != "private/README.md":
            violations.append(Violation(rel, "private 폴더 파일이 추적됨"))
        ...
        if lower.endswith(RAW_DATA_SUFFIXES):
            violations.append(Violation(rel, "원자료 형식(csv·엑셀·parquet) 파일이 추적됨"))
