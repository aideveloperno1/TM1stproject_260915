# `tests/fixtures/evidence/` — 근거 파일 경계 사례 22개

> 최신화: 2026-09-17

## 이 폴더는 무엇인가

근거 파일을 읽고 계산하는 코드가 **까다로운 상황을 모두 올바르게 처리하는지** 확인하려고 만든 가짜(합성) 근거 파일 22개입니다.
정상적으로 읽혀야 하는 파일 11개와, 일부러 틀리게 만들어 **오류로 거부돼야 하는** 파일 11개가 있습니다. 모든 수치는 가상 규모입니다.
...
- 스크립트는 월 수치로 직접 만드는 10개와, 정상 사례 `amount_up_share_down`을 만든 뒤 **한 곳만 바꾸는** 12개로 나뉜다. 한 곳만 바꾼 12개 중 `denominator_zero`만 로드에 성공하고 나머지 11개는 오류 문구 정확히 1개를 확인한다. 아래 표는 **로드 결과** 기준으로 나눴다
