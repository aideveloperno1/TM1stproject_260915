...
def test_adopted_option_adds_lines_and_records_change():
    ...
    assert first.evidence_ids == ("DEMO-R07-NATIONAL",)
...
def test_evidence_refs_come_from_used_records():
    ...
    assert [ref.evidence_id for ref in document.evidence_refs] == ["DEMO-R07-NATIONAL"]
    ref = document.evidence_refs[0]
    assert ref.scope_label == "전국 참고 — 특정 지역의 진단이 아님"
