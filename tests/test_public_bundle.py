...
def test_detects_private_env_real_name_and_raw_data(tmp_path: Path):
    for rel in ("private/README.md", ".env.example", "private/x.txt", ".env", ".env.local", "a_real_b.txt", "data.csv"):
        write(tmp_path, rel, "x")
    found = reasons(tmp_path, ["private/README.md", ".env.example", "private/x.txt", ".env", ".env.local", "a_real_b.txt", "data.csv"])
    assert found == [
        "private/x.txt: private 폴더 파일이 추적됨",
        ".env: 비밀 설정 파일이 추적됨",
        ".env.local: 비밀 설정 파일이 추적됨",
        "a_real_b.txt: 실제 자료 이름 규칙(_real_) 파일이 추적됨",
        "data.csv: 원자료 형식(csv·엑셀·parquet) 파일이 추적됨",
    ]
