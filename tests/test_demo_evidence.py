...
from policy_signal_map.config import DEFAULT_EVIDENCE_PATH, load_settings
...
def test_default_settings_point_to_demo_file_and_it_loads_without_warnings():
    ...
    assert result.file.dataset_version == "demo-001"
...
def test_demo_file_matches_generator():
    assert DEFAULT_EVIDENCE_PATH.read_text(encoding="utf-8") == to_json_text(build_demo_evidence.build())
