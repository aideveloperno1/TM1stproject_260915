...
def test_step_two_shows_tiny_change_label():
    assert "4→5월 +2.76% +2.74% 0.01%p 미만 (증가) 방향 같음" in text_of(reviewed_client().get("/step/2").text)
...
def test_step_three_placeholder_still_reachable():
    assert "다음 작업에서 구현할 화면입니다" in reviewed_client().get("/step/3").text
