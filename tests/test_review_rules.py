...
def test_direct_indicator_use_asks_question():
    ...
    assert r07.evidence_ids == ("DEMO-R07-NATIONAL",)
...
def test_region_note_and_scope_label_match_evidence_screen():
    r07 = outcome(run_review(plan(), DEMO), "R07")
    assert r07.scope_label == "전국 참고 — 특정 지역의 진단이 아님"
    assert r07.region_note == "전국 참고 자료입니다. 선택한 강원특별자치도 강릉시의 소비를 진단한 결과가 아닙니다."
...
def test_r06_and_r02_are_not_reviewed():
    result = run_review(plan(), DEMO)
    not_reviewed = {n.rule_id: n for n in result.not_reviewed}
    assert set(not_reviewed) == {"R06", "R02"}
    assert not_reviewed["R06"].scope_label == "분석 예시"
    assert "지역 기준" in not_reviewed["R02"].reason
    assert "R02" not in [o.rule_id for o in result.outcomes]
...
def test_outcome_order_follows_run_order():
    ...
    assert [o.rule_id for o in result.outcomes] == ["R07", "R03", "R01", "R05"]
