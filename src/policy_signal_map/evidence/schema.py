...
FILE_FIELDS = ("schema_version", "dataset_version", "records")
RECORD_FIELDS = (
    "evidence_id",
    "data_kind",
    "scope",
...
def parse_evidence(data: Any) -> ParseResult:
    issues = _Issues()
    file = _parse_file(data, issues)
    if issues.errors:
        file = None
    return ParseResult(file, tuple(issues.errors), tuple(issues.warnings))


def _parse_file(data: Any, issues: _Issues) -> EvidenceFile | None:
    where = ("파일",)
    ...
    records = [_parse_record(raw, index, issues) for index, raw in enumerate(raw_records)]
...
def _parse_record(raw: Any, index: int, issues: _Issues) -> EvidenceRecord | None:
    ...
    applicability = _parse_applicability(raw, where, issues)
...
def _parse_scope(raw: dict, where: tuple[str, ...], issues: _Issues) -> Scope | None:
    ...
    if geographic == "sido" and region_key == NATIONAL_REGION_KEY:
        issues.error((*where, "region_key"), f'시도 범위의 region_key는 "{NATIONAL_REGION_KEY}"일 수 없습니다')
...
def _parse_applicability(raw: dict, where: tuple[str, ...], issues: _Issues) -> Mapping[str, Applicability] | None:
