...
| `config.py` | 예정 (evidence 작업 시) | 환경변수 읽기. 아래 표 참고 |
| `.env.example` → 저장소 루트 | 예정 | `config.py` 항목 예시. 실제 `.env`는 git 제외 |
...
- 설정은 앱 시작 시 한 번 읽어 불변 객체(dataclass frozen)로 둔다.
- 실제 근거 파일(`data_kind: real`) + `PSM_LLM_PROVIDER=cloud` 조합이면 시작 시 오류로 멈춘다 (checks.md 합의).
