...
|---|---|---|
| `__init__.py` | — | — |
| `input.py` | `GET /`, `GET·POST /step/1`, `POST /reset` | `steps/input.html` |
...
- `needs_recheck` 선택은 상단에 모아 "원안 변경으로 재확인 필요" 표시. 가장 최근 원안 변경으로 더 이상 해당하지 않게 된 질문의 선택(`choice_set.last_archived`)만 안내한다 (6-5e)
