...
| `evidence_view.py` | 있음 | 2단계 화면용 데이터: 레코드 선택(전국), 금액 단위, 적용 상태별 표시 여부, 요약 문장, 차트 데이터(JSON 형식만), 보류 사례(합성 파일만), 해석 한계를 `data_limitations`(근거 파일, 분석 담당 작성)와 `service_notes`(서비스 고정 원칙)로 나눠 전달 |
